...
class AdvancedFileList extends React.Component {
  ...
  handleMenuItemOnClick(e, index) {
    ...
    if (e.shiftKey && selectedIndices.size) {
      const previousMax = Math.max.apply(Math, Array.from(selectedIndices.values()))
      let indicesToAdd = []
  ...
  render() {
    ...
    return (
      <div
        ...
        style={{border: '1px solid black', lineHeight: '1.2', tableLayout: 'fixed', height: '60vh', minWidth: '42vw'}}
        onKeyDown={ e => this.handleSelectAll(e) }
      >
        <div style={{display: files.length ? 'table-header-group' : 'none', minWidth: '100%', color: '#191919', fontWeight: 600, background: '#F5F4F4'}}>
        ...
        { files.map( (file,index) => {
            ...
            return (
              <div
                key={ 'afl-item' + index }
                style={{
                  fontSize: '0.9em', border: '1px solid #d8d8d8', marginLeft: 10, marginRight: 10, display: 'table-row', cursor: 'pointer',
                  background: selected ? '#4682b4' : '#fff', color: selected ? '#fff' : '#000', verticalAlign: 'middle', height: 20,
                  lineHeight: 3, overflowY: 'auto',
                }}
                onClick={ e => this.handleMenuItemOnClick(e, index) }
                onDoubleClick={ e => { e.preventDefault() }}
              >
                <div style={columnStyle}>{ file.name }</div>
                <div style={columnStyle}>{ this.getSizeFromBytes(file.fileSize) }</div>
                <div style={columnStyle}> { file.ext }</div>
                <div style={columnStyle}>{ moment(file.updated).format('YYYY-MM-DD HH:mm:ss').toString() }</div>
                {
                  downloadButton
                  ?  <div style={{verticalAlign: 'middle', display: 'table-cell', width: 50, paddingLeft: 10, background: '#fff'}}>
                      <MdDownload style={{marginTop: 1, fill: '#2196F3', verticalAlign: 'middle'}} onClick={ e => this.handleDownloadFile(e, file.name)}/>
                    </div>
                  : null
                }
              </div>
