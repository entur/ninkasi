...
interface ModalEditEntityTypeProps {
  isModalOpen: boolean;
...
const ModalEditEntityType = ({
  isModalOpen,
  handleCloseModal,
  handleSubmit,
  entityType: entityTypeProp,
  codeSpaces,
}: ModalEditEntityTypeProps) => {
  const classificationsRef = useRef<HTMLSelectElement | null>(null);

  const initial = entityTypeProp
    ? { ...entityTypeProp, classifications: entityTypeProp.classifications || [] }
    : initialEntityType;

  const [entityType, setEntityType] = useState<EntityType>(initial);
  ...
  useEffect(() => {
    if (entityTypeProp) {
      setEntityType({
        ...entityTypeProp,
        classifications: entityTypeProp.classifications || [],
      });
    }
  }, [entityTypeProp]);
