...
class ModalCreateUser extends React.Component {
  ...
  render() {
    ...
    const { isModalOpen, handleSubmit, takenUsernames, organizations, responsibilities } = this.props
      ...
      <Modal isOpen={isModalOpen} onClose={() => this.handleOnClose()} minWidth="35vw" minHeight="auto">
        <div>
          ...
          <div style={{display: 'flex', justifyContent: 'space-between', marginRight: 15}}>
            ...
            <RaisedButton disabled={invalidPrivateCode} label="Create" primary={true} onClick={ () => handleSubmit(user)} />
          </div>
        </div>
      </Modal>
    )
