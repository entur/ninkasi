    ...
    return (
      <div style={{display: 'flex', flexDirection: 'column', width: '80%', margin: 'auto', marginTop: '5%'}}>
        <Dropzone
          ...
          <div style={{textAlign: 'center'}}>Try dropping some files here, or click to select files to upload.</div>
        </Dropzone>
        { files.length
          ?
        ...
        <Button disabled={!files.length} style={uploadButtonStyle} color="primary" onClick={ () => files.length && handleFileUpload(files) }>Upload</Button>
      </div>
    )
  }
